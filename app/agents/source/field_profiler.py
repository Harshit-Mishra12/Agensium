...
def _profile_dataframe(df: pd.DataFrame, baseline_schema: dict | None = None):
    """
    Profile a single dataframe (field stats, anomalies, drift).
    """
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(col_data):
            stats = {
                ...
                "entropy": float(entropy(pd.value_counts(col_data, normalize=True), base=2)),
            }
...
def profile_sql(contents: bytes):
    """Profile all tables in a .sql file (SQLite dump)."""
